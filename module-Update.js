    ...
    switch (X.name) {
        case "进度":
            ui.run(() => {
                ...
                view.更新安装.setText("下载中 " + X.data + "%");
            })
            break;
        case "结果":
            if (X.data == "下载完成") {
              view.更新安装.setText("下载完成");
            }
            break;
    }
});

    var 版本=storage.get('版本');
    var 更新内容=storage.get('更新内容');
    var 体积=storage.get('体积');
    let 升级内容 = ["版本:"+版本, "增量包体积:"+体积, "", "更新内容:", 更新内容].join("\n");
